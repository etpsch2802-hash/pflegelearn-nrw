...
export default async function handler(req, res) {
  ...
  try {
    const r = await fetch(SB_URL + '/rest/v1/leads?on_conflict=email', {
      method: 'POST',
      headers: {
        'apikey': SB_SERVICE,
    ...
    // 201 = neu, 200/204 = ok, 409 = bereits vorhanden (alles als Erfolg werten)
    if (!r.ok && r.status !== 409) {
      const t = await r.text();
      console.error('[lead] supabase', r.status, t);
      res.status(500).json({ error: 'store' }); return;
    }
